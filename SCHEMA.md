...
|---|---|---|---|
| `id` | string | ✓ | ULID |
| `name` | string | ✓ | 表示名。例: `"田中さん"` `"@tonalobserver"` |
| `kind` | enum | ✓ | `self` / `pianist` / `commenter` / `other` |
| `url` | string | | プロフィールURL（X等）。索引あり。同一URLは重複作成しない |
| `provider` | enum | | 由来。`x` / `manual` / `unknown`（既定 `manual`） |
| `handle` | string | | 旧フィールド（任意・後方互換のため残置） |
| `note` | string | | 傾向メモなど |
| `createdAt` | string | ✓ | ISO 8601 |

> **X プロフィールURL自動入力**: Person URL欄に `x.com/username` 等を貼ると、Person名が空のとき
> `@username` を仮入力し、可能なら表示名へ置換（X oEmbed、失敗時は `@username` のまま）。手入力は上書きしない。
> **互換性**: 旧Person（`url`/`provider` 無し）は読込・import時に `url:""` / `provider:"manual"` で補完
> （Dexie v3 マイグレーション＋import補完）。`handle` は残置。
...
| `focus` | enum | ✓ | **観察対象**（何を聴いていたか）。既定 `performance`。下表参照。索引あり |
| `personId` | string | | → Person（省略可） |
| `sourceId` | string | | → Source（省略可） |
| `contrastTermId` | string | | → Term。**任意**。その時の反対極（人ごとの対比） |
| `timestamp` | string | | 任意。例 `"2:14"` `"1:03:22"`。空文字は実質未保存（表示時に非表示） |
| `locationNote` | string | | 任意。例 `"ppの入り"` `"高音の余韻"` `"再現部"` |
...
### focus（観察対象）の候補
後からソート・集計するための分類軸。クイック入力でチップ選択、既定は `performance`。

| key | 表示 | 意味 |
|---|---|---|
| `performance` | 演奏全体 | 曲・演奏全体の印象 |
| `instrument` | 楽器 | 楽器そのものの音色 |
| `interpretation` | 解釈 | 演奏者の解釈・表現 |
| `recording` | 録音 | 録音・マイク・ミックス |
| `acoustics` | 空間 | ホール・空間・残響 |
| `moment` | 瞬間 | 特定の瞬間・局所（選ぶと timestamp / locationNote 欄が自動で開く） |

> **互換性**: `focus` 未設定の旧データは読込時に `performance` として扱う（Dexie v2 マイグレーション
> ＋ import 時の補完）。`timestamp` / `locationNote` が無い場合は未表示。
...
```json
{
  ...
  "utterances": [
    { "id": "U1", "termId": "T_akarui", "valence": "positive", "focus": "moment", "personId": "P_tanaka", "sourceId": "S_yt41",
      "contrastTermId": "T_komotta", "timestamp": "2:14", "locationNote": "ppの入り", "aspect": "高音", "observedVia": "direct", "createdAt": "2026-06-12T01:00:00Z" },
    { "id": "U2", "termId": "T_akarui", "valence": "negative", "focus": "performance", "personId": "P_cmt", "sourceId": "S_yt41",
      "contrastTermId": "T_kinkin", "timestamp": "", "locationNote": "", "note": "明るすぎてキンキンする", "observedVia": "youtube_comment", "createdAt": "2026-06-12T01:05:00Z" },
    { "id": "U3", "termId": "T_nuke", "valence": "positive", "focus": "instrument", "personId": "P_self", "sourceId": "S_yt41",
      "timestamp": "", "locationNote": "", "observedVia": "direct", "createdAt": "2026-06-12T01:10:00Z" }
  ]
}
```
